...
def f(n: str) -> None:
    ...
    num_code = "".join(filter(lambda x: "from lib import" not in x, open(f"{ROOT_PATH[n]}/{n}.py", encoding="utf-8", errors='ignore').readlines()))
    lib_code = open(f"{ROOT_PATH[n]}/lib.py", encoding="utf-8", errors='ignore').read()
    code = lib_code + "\n\n\n" + num_code
